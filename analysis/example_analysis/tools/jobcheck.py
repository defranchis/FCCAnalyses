...
#######################################################
# Check the log files of slurm/condor jobs for errors #
#######################################################
# Use case:
#   Use this tool for automated checking of job failures.
#   The check happens on the basis of the log files
#   that are created by the slurm/condor job scheduler.
...
if __name__=='__main__':
    ...
    # find files
    print('Finding files...')
    condorpattern = os.path.join(args.dir, 'cjob_*_err_*')
    slurmpattern = os.path.join(args.dir, 'sjob_*.out')
    files = glob.glob(condorpattern) + glob.glob(slurmpattern)
    nfiles = len(files)
