...
from tools.histtools import cliphistogram
from tools.histtools import clonehistogram
from tools.histtools import scalehistogram
from tools.histtools import addhistograms
from tools.histtools import binperbinmaxvar
from tools.histtools import rootsumsquare
...
class ProcessInfo(object):
  ...
  def __init__( self, name,
    ...
    # - histname: name of the nominal histogram
    # - systematics: dict mapping systematics names to impacts;
    #   the values of this dict can have the following types:
    #     - str '-' (if the systematic is not applicable)
    #     - float (for flat systematics)
    #     - tuple of two names of histograms (up and down respectively).
    # note: this class does not contain the actual histogram objects.
    #       only the info needed to retrieve them.
    self.name = name
    self.pid = pid
    self.pyield = pyield
...
class Process(object):
  ...
  def __init__( self, info, histdict, doclip=False ):
    ### initializer
    # input arguments:
    # - info: an instance of type ProcessInfo
    # - histdict: dictionary with histograms
    self.info = info
    self.hist = None
    self.systhists = {}
    keylist = list(histdict.keys())
    # read nominal histogram
    if not self.info.histname in keylist:
      msg = 'ERROR in Process.init:'
      msg += ' nominal histogram "{}" not found in provided histograms'.format(self.info.histname)
      raise Exception(msg)
    self.hist = histdict[self.info.histname]
    # clip histogram if requested
    if doclip: self.hist = cliphistogram(self.hist)
    # read systematic histograms
    for systematic,val in self.info.systematics.items():
      if( isinstance(val,str) and val=='-' ):
        # use nominal
        uphist = clonehistogram(self.hist)
        downhist = clonehistogram(self.hist)
        self.systhists[systematic] = (uphist,downhist)
      elif( isinstance(val,float) ):
        # use nominal scaled by a factor
        uphist = clonehistogram(self.hist)
        uphist = scalehistogram(uphist, val)
        downhist = clonehistogram(self.hist)
        downhist = scalehistogram(downhist, 2-val)
        self.systhists[systematic] = (uphist,downhist)
      elif( isinstance(val,tuple) ):
        # read up-histogram
        if not val[0] in keylist:
          msg = 'ERROR in Process.init:'
          msg += ' histogram "{}" not found in provided histograms'.format(val[0])
          raise Exception(msg)
        uphist = histdict[val[0]]
        if doclip: uphist = cliphistogram(uphist)
        # read down-histogram
        if not val[1] in keylist:
          msg = 'ERROR in Process.init:'
          msg += ' histogram "{}" not found in provided histograms'.format(val[1])
          raise Exception(msg)
        downhist = histdict[val[1]]
        if doclip: downhist = cliphistogram(downhist)
        self.systhists[systematic] = (uphist,downhist)
  ...
  def get_systematic( self, systematic, idx, diff=False, absolute=False ):
    ...
    if diff:
      diffhist = addhistograms(hist, self.hist, factor=-1)
      if absolute: diffhist = (np.abs(diffhist[0]), diffhist[1])
    return hist
  ...
  def get_systematics_rss( self, systematics='all' ):
    ...
    if( isinstance(systematics, list) and len(systematics)==0 ):
      return np.zeros(self.hist[0].shape)
    ...
    for systematic in systematics:
      ...
      maxhist = binperbinmaxvar( [uphist,downhist], self.hist )
      maxhistlist.append(maxhist)
    # add resulting histograms in quadrature
    syshist = rootsumsquare(maxhistlist)
    return syshist
  ...
  def fromhistlist( histnames, variable, signals=[],
    ...
    if adddata:
      datahistname = ([el for el in histnames
                        if (variable in el
                        and el.split(variable)[0].strip('_')==datatag)])
...
class ProcessCollection(object):
  ...
  def __init__( self, info, histdict, doclip=False ):
    ### initializer from a ProcessInfoCollection and a dict containing the histograms
    ...
    for pname,pinfo in self.info.pinfos.items():
      self.processes[pname] = Process( pinfo, histdict, doclip=doclip )
    self.datahist = None
    if self.info.datahistname is not None:
      self.datahist = histdict[self.info.datahistname]
  ...
  def get_hist_sum( self, histlist ):
    ...
    if len(histlist)==1: return clonehistogram(histlist[0])
    sumhist = clonehistogram(histlist[0])
    for hist in histlist[1:]:
        sumhist = addhistograms(sumhist, hist)
    return sumhist
  ...
  def get_yields( self, systematic=None ):
    ...
    if systematic is None:
      ...
      nominalhist = self.get_nominal()
      res['total'] = np.sum(nominalhist[0])
      return res
    else:
  ...
  def get_difference_up( self, systematic, processes='all' ):
    ...
    uphist = addhistograms( uphist, self.get_nominal(), factor=-1 )
    return uphist
  ...
  def get_difference_down( self, systematic, processes='all' ):
    ...
    downhist = addhistograms( downhist, self.get_nominal(), factor=-1 )
    return downhist
  ...
  def get_systematics_rss( self, systematics='all', processes='all',
    ...
    if( len(systematics)==0 ):
      return np.zeros(self.get_nominal()[0].shape)
    
    # case of uncorrelated processes:
    # root sum square over both systematics and processes
    if not correlate_processes:
      ...
      return rootsumsquare( per_process_rss )

    # correlated processes:
    # linear sum over processes, quadratic over systematics
    else:
      maxhistlist = []
      for s in systematics:
        ...
        nominalhist = self.get_nominal()
        maxhist = binperbinmaxvar( [uphist,downhist], nominalhist )
        maxhistlist.append(maxhist)
      return rootsumsquare( maxhistlist )
