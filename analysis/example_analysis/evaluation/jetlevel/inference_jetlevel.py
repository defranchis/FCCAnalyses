...
from evaluation.inferencetools import infer_jets
from evaluation.jetlevel.inferencetools_jetlevel import add_variables_jetlevel
...
if __name__=='__main__':
    ...
    newvarnames = add_variables_jetlevel(None, names_only=True)
    ...
    for _, files in sampledict.items():
        for fidx, file in enumerate(files):
            ...
            for bidx, batch_start_index in enumerate(batch_start_indices):
                ...
                add_variables_jetlevel(events[key])

                # do inference
                scores = infer_jets(events[key], args.model, prepdict,
                            batch_size=1000)
